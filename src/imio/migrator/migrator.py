...
class Migrator(object):
    ...
    def is_in_part(self, part):
        ...
        if self.run_part == part:
            ...
            self.log_mem(
                "PART {}".format(part)
            )  # print intermediate part memory info if run in one step
            return True
        return False
    ...
    def log_mem(self, tag=""):
        ...
        if self.display_mem:
            logger.info(
                "Mem used {} at {}, ({})".format(process_memory(), tag, memory())
            )
    ...
    def refreshDatabase(
        ...
        if workflows:
            logger.info(
                "Refresh workflow-related information on every object of the database..."
            )
            if not workflowsToUpdate:
                ...
                for wf_id in workflowsToUpdate:
                    logger.info(
                        'Refreshing workflow(s) "{0}"...'.format(
    ...
    def cleanRegistries(self, registries=("bundles_registry", "portal_setup")):
        """
        Clean p_registries, remove not found elements.
        """
        logger.info("Cleaning registries...")
        if "bundles_registry" in registries:
            bundles = self.registry.collectionOfInterface(
                IBundleRegistry, prefix="plone.bundles", check=False
            )
            for bundle_name, bundle in bundles.items():
                js = bundle.jscompilation
                css = bundle.csscompilation
                resource_is_missing = not self.portal.restrictedTraverse(
                    js, False
                ) or not self.portal.restrictedTraverse(css, False)
                if resource_is_missing:
                    # we found a notFound resource (css or js), remove it
                    logger.info(f"Removing {bundle_name} from bundle registry")
                    del bundles[bundle_name]
            logger.info("Bundle registry has been cleaned!")
    ...
    def removeUnusedPortalTypes(self, portal_types=[]):
        ...
        logger.info(
            "Removing no more used {0} portal_types...".format(", ".join(portal_types))
        )
        # remove from portal_types
        types = self.portal.portal_types
        to_remove = [
            portal_type for portal_type in portal_types if portal_type in types
        ]
        if to_remove:
            types.manage_delObjects(ids=to_remove)
        # remove from registry plone.types_not_searched
        nsTypes = list(self.registry.get("plone.types_not_searched"))
        for portal_type_id in portal_types:
            if portal_type_id in nsTypes:
                nsTypes.remove(portal_type_id)
        self.registry["plone.types_not_searched"] = tuple(nsTypes)
        logger.info("Done.")
    ...
    def reindexIndexes(
        self, idxs=[], update_metadata=False, meta_types=[], portal_types=[]
    ):
    ...
    def reindexIndexesFor(self, idxs=[], **query):
        ...
        pghandler.info(
            'In reindexIndexesFor, reindexing indexes "{0}" on "{1}" objects ({2})...'.format(
                ", ".join(idxs) or "*", len(brains), str(query)
            )
        )
    ...
    def reinstall(self, profiles, ignore_dependencies=False, dependency_strategy=None):
        ...
        logger.info(
            "Reinstalling product(s) %s..."
            % ", ".join(
                [
                    profile.startswith("profile-") and profile[8:] or profile
    ...
    def upgradeProfile(self, profile, olds=[]):
        ...
        def run_upgrade_step(step, source, dest):
            logger.info(
                "Running upgrade step %s (%s -> %s): %s"
                % (profile, source, dest, step.title)
            )
            step.doStep(self.ps)
        ...
        for container in upgrades:
            if isinstance(container, dict):
                if not olds or container["sdest"] in olds:
                    applied_dests.append(
                        (normalize_version(container["sdest"]), container["sdest"])
                    )
                    run_upgrade_step(
                        container["step"], container["ssource"], container["sdest"]
                    )
            elif isinstance(container, list):
                for dic in container:
                    if not olds or dic["sdest"] in olds:
                        applied_dests.append(
                            (normalize_version(dic["sdest"]), dic["sdest"])
                        )
                        run_upgrade_step(dic["step"], dic["ssource"], dic["sdest"])
        if applied_dests:
            current_version = normalize_version(
                self.ps.getLastVersionForProfile(profile)
            )
    ...
    def upgradeAll(self, omit=[]):
        ...
        for profile in self.ps.listProfilesWithUpgrades():
            ...
            if (
                profile not in omit
                and self.ps.getLastVersionForProfile(profile) != "unknown"
            ):
                self.upgradeProfile(profile)

    def runProfileSteps(
        self, product, steps=[], profile="default", run_dependencies=False
    ):
        ...
        for step_id in steps:
            logger.info(
                "Running profile step '%s:%s' => %s" % (product, profile, step_id)
            )
            self.ps.runImportStepFromProfile(
                "profile-%s:%s" % (product, profile),
                step_id,
